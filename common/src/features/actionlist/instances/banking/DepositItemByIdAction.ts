...
export class DepositItemByIdAction extends Action {
    icon: string;
    id = ActionId.DepositItemByIdAction;
    ...
    constructor(itemId: ItemId, amount: number) {
        super(`Deposit ${amount} ${itemId}`, 5);
        this.itemId = itemId;
        this.amount = amount;
    }

    gainReward(): boolean {
        this._bank.depositByItemId(this.itemId, this.amount);
        return false;
    }
